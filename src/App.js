...
function App() {
  ...
  async function searchData() {
    try{
      let productData = await axios.get(`https://web-scrapping-hd63.onrender.com/api/product/mobiles?search=${search}&page=1&size=${size}`);
  ...
  useEffect(() => {
    ...
    searchData();
  }, [search, size]) //eslint-disable-line react-hooks/exhaustive-deps
 
  // pagination
  const fetchComments = async (currentPage) => {
    try {
      let productData = await axios.get(
        `https://web-scrapping-hd63.onrender.com/api/product/mobiles?search=${search}&page=${currentPage}&size=${size}`
      );
